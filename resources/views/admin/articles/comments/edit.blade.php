...
@section('content')
    <div class="layui-row" style="padding: 2px 15px 2px 15px">
        <br>
        <form class="layui-form layui-form-pane" lay-filter="edit" method="post" action="{{url('admin/comments/'.$commentsEdit->id)}}">
            {{csrf_field()}}
            {{method_field('PUT')}}
            <input type="hidden" value="{{$commentsEdit->id}}" name="id">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="to_uid"  placeholder="选填必须数字" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
...
@section('js')
    <script>
        layui.use(['form', 'layedit', 'laydate'], function(){
            ...
            //初始值
            form.val("edit", {
                "topic_id": "{{$commentsEdit->topic_id}}"
                ,"topic_type": "{{$commentsEdit->topic_type}}"
                ,"from_uid": "{{$commentsEdit->from_uid}}"
                ,"to_uid": "{{$commentsEdit->to_uid}}"
                ,"comments_pid": "{{$commentsEdit->comments_pid}}"
                ,"content": "{{$commentsEdit->content}}"
            })
        });
    </script>
@endsection
