                ...
                switch(obj.event){
                    ...
                    case 'add':
                        layer.open({
                            ...
                            area: ['893px', '100%'],
                            content: '{{url("/admin/goodsorder/create")}}',
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除此分类吗？', function(index){
                        $.ajax({
                            type: "POST",
                            url: "{{url('/admin/goodsorder')}}/"+data.id,
                            cache: false,
                            data:{_method:"DELETE", _token: "{{csrf_token()}}","buycount":data.buycount,"goods_id":data.goods_id},
                   ...
                   layer.open({
                        ...
                        maxmin: true, //开启最大化最小化按钮
                        area: ['893px', '100%'],
                        content: '{{url("/admin/goodsorder")}}/'+ data.id + '/edit',
                       cancel: function(index, layero){
                           // 刷新表格
                           tableIns.reload();
                           return true;
