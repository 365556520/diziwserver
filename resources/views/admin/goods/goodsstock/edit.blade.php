...
@section('content')
    ...
    <div class="layui-row" style="padding: 2px 15px 2px 15px">
        <br>
        <form class="layui-form layui-form-pane" lay-filter="edit" method="post" action="{{url('admin/goodsstock/'.$gsEdit->id)}}">
            {{csrf_field()}}
            {{method_field('PUT')}}
            <input type="hidden" value="{{$gsEdit->id}}" name="id">
            {{--用户id--}}
            <input type="hidden" value="{{$gsEdit->user_id}}" name="user_id">
